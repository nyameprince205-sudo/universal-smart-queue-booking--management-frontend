...
import { Ticket, CheckCircle2, UserX, Calendar, XCircle, CalendarCheck, UserPlus, Activity } from "lucide-react";
...
function ActivityFeed() {
  ...
  useEffect(() => {
    listRecentActivity().then(setActivity).catch(err => setError(err.response?.data?.error || "Couldn't load recent activity."));
  }, []);
  return <div className="bg-white rounded-lg border border-slate-200 p-5">
      ...
      {activity && activity.length > 0 && <ul className="space-y-3">
          {activity.map(entry => {
