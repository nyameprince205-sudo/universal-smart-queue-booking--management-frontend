...
function CreateBookingForm({
  ...
  async function handleLookup(e) {
    ...
    try {
      ...
      if (err.response?.status === 404) setNotFound(true);else setError(err.response?.data?.error || "Couldn't look up that number.");
    }
  }
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!customer) customer = await quickRegisterCustomer({
        name,
        phone
      });
      await onSubmit({
  ...
  return <form onSubmit={foundCustomer || notFound ? handleSubmit : handleLookup} className="space-y-4">
      ...
      {notFound && <div>
          ...
          <input type="text" required placeholder="Customer name" value={name} onChange={e => setName(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-800" />
        </div>}
      ...
      {(foundCustomer || notFound) && <>
        ...
        setServiceId("");
          ...
          <select required disabled={!branchId} value={serviceId} onChange={e => setServiceId(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-800 disabled:bg-slate-50">
          ...
          <button type="submit" disabled={!ready || submitting} className="w-full rounded-md bg-sky-600 text-white py-2 text-sm font-medium hover:bg-sky-500 disabled:opacity-50 transition-colors">
...
function BookingsPage() {
  ...
  return <div className="p-4 sm:p-8 max-w-4xl">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <h1 className="text-xl sm:text-2xl font-semibold text-slate-800">Bookings</h1>
      ...
      {!loading && bookings.length > 0 && <div className="mt-6 sm:hidden space-y-3">
          {bookings.map(b => <div key={b.id} className="bg-white rounded-lg border border-slate-200 p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-medium text-slate-800 truncate">{b.customer?.name}</p>
                  <p className="text-xs text-slate-400">{b.customer?.phone}</p>
                </div>
                <span className={`rounded-full px-2 py-0.5 text-xs font-medium shrink-0 ${STATUS_STYLES[b.status] || "bg-slate-100 text-slate-500"}`}>
                  {b.status.replace("_", " ")}
                </span>
              </div>
              <p className="mt-2 text-sm text-slate-600">{b.service?.name}</p>
      ...
      {!loading && bookings.length > 0 && <div className="mt-6 bg-white rounded-lg border border-slate-200 overflow-hidden hidden sm:block">
      ...
      {showAddModal && <Modal title="Create Booking" onClose={() => setShowAddModal(false)}>
