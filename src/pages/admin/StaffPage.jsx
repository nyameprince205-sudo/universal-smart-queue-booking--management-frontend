...
function CreateStaffForm({
  ...
  return <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input type="email" required value={email} onChange={e => setEmail(e.target.value)} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-slate-800" />
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-700">Initial password</label>
        <input type="password" required value={password} onChange={e => setPassword(e.target.value)} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-slate-800" />
      ...
      <div>
        ...
        <select value={branchId} onChange={e => setBranchId(e.target.value)} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-slate-800">
...
function StaffPage() {
  ...
  async function handleToggleStatus(member) {
    ...
    setError(null);
    try {
      if (isActive) await deactivateStaff(member.id);else await reactivateStaff(member.id);
      await loadAll();
    } catch (err) {
  ...
  return <div className="p-8 max-w-4xl">
      ...
      {!loading && staff.length > 0 && <div className="mt-6 bg-white rounded-lg border border-slate-200 overflow-hidden">
          <table className="w-full text-sm text-left">
            ...
            return <tr key={s.id} className={isActive ? "" : "bg-slate-50"}>
                    <td className="px-4 py-3 font-medium text-slate-800">{s.name}</td>
                    <td className="px-4 py-3 text-slate-500">{s.email}</td>
                    <td className="px-4 py-3 text-slate-500">{s.branchName || "Not branch-scoped"}</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${isActive ? "bg-green-100 text-green-700" : "bg-slate-200 text-slate-600"}`}>
