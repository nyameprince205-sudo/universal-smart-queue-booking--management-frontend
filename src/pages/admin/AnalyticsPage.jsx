...
import { getServicePopularity, getPeakHours, getBookingTrends, getStaffPerformance, getBranchComparison, getRevenueReport } from "../../api/analytics";
...
function OverviewTab({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    Promise.all([getPeakHours(filters), getServicePopularity(filters)]).then(([hoursData, servicesData]) => {
      setPeakHours(hoursData);
      setServices(servicesData);
    }).catch(err => setError(err.response?.data?.error || "Couldn't load this data.")).finally(() => setLoading(false));
  }, [filters]);
  ...
  return <div className="mt-6 space-y-6">
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg border border-slate-200 p-5">
          ...
          {mostPopular.length === 0 ? <p className="text-sm text-slate-400">No bookings in this range.</p> : <ul className="space-y-2">
              {mostPopular.map(s => <li key={s.serviceId} className="flex items-center justify-between text-sm">
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-5">
          ...
          {leastPopular.length === 0 ? <p className="text-sm text-slate-400">No bookings in this range.</p> : <ul className="space-y-2">
              {leastPopular.map(s => <li key={s.serviceId} className="flex items-center justify-between text-sm">
                  <span className="text-slate-700">{s.serviceName}</span>
                  <span className="font-medium text-slate-800">{s.bookingCount}</span>
                </li>)}
            </ul>}
...
function TrendsTab({
  ...
  return <div className="mt-6">
      ...
      {trend && <div className="bg-white rounded-lg border border-slate-200 p-5">
          {trend.trend.length === 0 ? <p className="text-sm text-slate-400 py-8 text-center">No bookings in this range.</p> : <ResponsiveContainer width="100%" height={280}>
            ...
            fill: "#64748b"
                ...
                <Tooltip />
                <Line type="monotone" dataKey="count" stroke="#0284c7" strokeWidth={2} dot={{
            r: 3
          }} />
              </LineChart>
            </ResponsiveContainer>}
...
function StaffBranchesTab({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    Promise.all([getStaffPerformance(filters), getBranchComparison(filters)]).then(([staffData, branchData]) => {
      setStaff(staffData);
  ...
  return <div className="mt-6 space-y-6">
      <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
        ...
        {staff.staff.length === 0 ? <p className="text-sm text-slate-400 px-5 pb-5">
            ...
            <tbody className="divide-y divide-slate-100">
              {staff.staff.map(s => <tr key={s.userId}>
      ...
      <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {branches.branches.map(b => <tr key={b.branchId}>
...
function RevenueTab({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    getRevenueReport(filters).then(setRevenue).catch(err => setError(err.response?.data?.error || "Couldn't load revenue.")).finally(() => setLoading(false));
  }, [filters]);
  if (loading) return <p className="mt-8 text-slate-400">Loading…</p>;
  if (error) return <div className="mt-6 rounded-md bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">{error}</div>;
  ...
  return <div className="mt-6 space-y-6">
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-5">
        ...
        {revenue.byService.length === 0 ? <p className="text-sm text-slate-400 py-4 text-center">No completed bookings with revenue in this range.</p> : <ResponsiveContainer width="100%" height={220}>
            <BarChart data={revenue.byService}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              <XAxis dataKey="serviceName" tick={{
            fontSize: 11,
            fill: "#64748b"
          }} />
              <YAxis tick={{
      ...
      {revenue.byDay.length > 0 && <div className="bg-white rounded-lg border border-slate-200 p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            ...
            fill: "#64748b"
              ...
              <Tooltip formatter={v => `GHS ${v}`} />
              <Line type="monotone" dataKey="revenue" stroke="#16a34a" strokeWidth={2} dot={{
            r: 3
          }} />
            </LineChart>
          </ResponsiveContainer>
