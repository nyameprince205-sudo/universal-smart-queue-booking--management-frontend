...
// Shared by both the "Add Branch" and "Edit Branch" forms below — same
// fields either way, just pre-filled differently and POST vs PATCH on
// submit. One component avoids maintaining two nearly-identical forms.
function BranchForm({ initialValues, onSubmit, submitLabel }) {
  ...
  async function handleSubmit(e) {
    ...
    setSubmitting(true);
    try {
      await onSubmit({ name, address, phone, timezone });
    } catch (err) {
      setError(err.response?.data?.error || "Something went wrong. Please try again.");
      setSubmitting(false);
    }
    // Deliberately not resetting `submitting` in a `finally` on the success
    // path — the parent closes this modal immediately on success, so
    // there's no more form left to re-enable.
  }
...
function BranchesPage() {
  ...
  const [editingBranch, setEditingBranch] = useState(null); // the branch object being edited, or null
  ...
  // Toggling status doesn't need the full form — it's a single-field PATCH,
  // so a plain button makes more sense than opening the edit modal just to
  // flip one value.
  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      {!loading && branches.length === 0 && !error && (
        <p className="mt-8 text-slate-400">No branches yet — add your first one.</p>
      )}
      ...
      {showAddModal && (
        <Modal title="Add Branch" onClose={() => setShowAddModal(false)}>
