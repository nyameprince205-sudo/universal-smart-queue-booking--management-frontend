...
function planActionLabel(plan, subscription, startingCheckoutPlanId) {
  if (subscription?.plan === plan.name) return "Current Plan";
  if (startingCheckoutPlanId === plan.id) return "Starting checkout…";
  if (!subscription?.hasAccess) return "Subscribe";
  if (subscription.isTrial) return "Subscribe";
  const currentPrice = Number(subscription.price || 0);
  const planPrice = Number(plan.price || 0);
  if (planPrice > currentPrice) return "Upgrade";
  if (planPrice < currentPrice) return "Switch to this plan";
  return "Switch";
}
      ...
      {!loading && <>
          ...
          <div className="mt-6 bg-white rounded-lg border border-slate-200 p-5">
            <p className="text-sm font-medium text-slate-500">
              {subscription?.isTrial ? "30-Day Access Period" : "Current Plan"}
            </p>
            {subscription && subscription.plan ? <div className="mt-2">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_STYLES[subscription.hasAccess ? subscription.status : "expired"] || "bg-slate-100 text-slate-500"}`}>
                ...
                {subscription.hasAccess ? <>
                    ...
                    <div className="mt-4 flex items-baseline gap-2">
                      <p className={`text-4xl font-semibold ${subscription.daysRemaining <= 3 ? "text-red-600" : subscription.daysRemaining <= 7 ? "text-amber-600" : "text-slate-800"}`}>
                    ...
                    <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
                      <div>
                        ...
                        <p className="text-slate-700 font-medium">{new Date(subscription.startDate).toLocaleDateString()}</p>
                      </div>
                      <div>
                        <p className="text-slate-400 text-xs">
                          {subscription.isTrial ? "Access ends" : "Next Billing Date"}
                        </p>
                    ...
                    <p className="text-sm text-red-700 mt-1">
                      Ended {new Date(subscription.endDate).toLocaleDateString()}. Choose a plan below
            ...
            return <div key={plan.id} className={`relative rounded-lg border p-5 ${isRecommended ? "border-sky-400 shadow-md" : "border-slate-200"} ${isCurrentPlan ? "bg-sky-50" : "bg-white"}`}>
                  ...
                  label: formatFeature(f)
                    ...
                    <button onClick={() => handleSubscribe(plan.id)} disabled={isCurrentPlan || startingCheckoutPlanId === plan.id} className="mt-4 w-full rounded-md bg-sky-600 text-white py-2 text-sm font-medium hover:bg-sky-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                      {planActionLabel(plan, subscription, startingCheckoutPlanId)}
                    </button>
                  </div>;
          })}
            </div>
