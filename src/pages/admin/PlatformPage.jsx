...
function OrganizationsTab({
  ...
  return <div>
              ...
              {organizations.map(org => <tr key={org.id}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <select value={org.status} disabled={updatingStatusId === org.id} onChange={e => onStatusChange(org, e.target.value)} className="text-xs rounded border border-warm-border px-1 py-0.5 text-warm-muted-2 disabled:opacity-50">
      ...
      {showAddModal && <Modal title="Add Organization" onClose={() => setShowAddModal(false)}>
...
function RequestsTab({
  ...
  return <div>
      <div className="flex gap-2">
        {["pending", "approved", "rejected", ""].map(s => <button key={s || "all"} onClick={() => setStatusFilter(s)} className={`px-3 py-1.5 rounded-md text-sm font-medium capitalize transition-colors ${statusFilter === s ? "bg-forest-600 text-white" : "bg-white text-warm-muted-2 border border-warm-border hover:bg-warm-bg"}`}>
            {s || "All"}
          </button>)}
      </div>
      ...
      <div className="mt-4 space-y-3">
        {requests.map(r => <div key={r.id} className="bg-white rounded-lg border border-warm-border p-5">
            ...
            {r.status === "approved" && r.createdOrganizationId && <p className="mt-2 text-sm text-forest-600">Organization created (id {r.createdOrganizationId}).</p>}

            {r.status === "pending" && approvingId !== r.id && <div className="mt-4 flex gap-2">
            ...
            {r.status === "pending" && approvingId === r.id && <div className="mt-4 rounded-md border border-warm-border bg-warm-bg p-3">
                ...
                <div className="flex gap-2">
                  <select value={selectedBusinessTypeId} onChange={e => setSelectedBusinessTypeId(e.target.value)} className="flex-1 rounded-md border border-warm-border px-2 py-1.5 text-sm text-warm-ink">
                    <option value="">Select a business type…</option>
                    {businessTypes.map(bt => <option key={bt.id} value={bt.id}>{bt.name}</option>)}
                  </select>
                  <button onClick={() => confirmApprove(r.id)} disabled={!selectedBusinessTypeId || reviewingId === r.id} className="rounded-md bg-emerald-700 text-white px-3 py-1.5 text-sm font-medium hover:bg-emerald-600 disabled:opacity-50 transition-colors whitespace-nowrap">
                    {reviewingId === r.id ? "Creating…" : "Confirm & create"}
                  </button>
                  <button onClick={() => setApprovingId(null)} className="rounded-md bg-white border border-warm-border text-warm-ink px-3 py-1.5 text-sm font-medium hover:bg-warm-bg transition-colors">
                    Cancel
                  </button>
                </div>
              </div>}
...
function MessagesTab({
  ...
  return <div>
      ...
      <div className="mt-4 space-y-3">
        {submissions.map(s => <div key={s.id} className={`rounded-lg border p-5 ${s.isRead ? "bg-white border-warm-border" : "bg-gold-50 border-gold-100"}`}>
            <div className="flex items-start justify-between">
              <div>
            ...
            {!s.isRead && <button onClick={() => onMarkRead(s.id)} disabled={markingId === s.id} className="mt-3 text-sm text-forest-600 hover:underline disabled:opacity-50">
  ...
  return <div>
      ...
      {!loading && orgAdmins.length > 0 && <div className="mt-4 bg-white rounded-lg border border-warm-border overflow-hidden">
          <table className="w-full text-sm text-left">
            ...
            return <tr key={a.id} className={isActive ? "" : "bg-warm-bg"}>
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => onToggleStatus(a)} disabled={actioningId === a.id} className={`text-sm font-medium hover:underline disabled:opacity-50 ${isActive ? "text-red-600" : "text-forest-600"}`}>
...
function PlatformPage() {
  ...
  const refreshBadgeCounts = useCallback(async () => {
    const [pending, unread] = await Promise.all([listOrganizationRequests("pending").catch(() => []), listContactSubmissions(true).catch(() => [])]);
  ...
  async function handleToggleOrgAdminStatus(admin) {
    ...
    setError(null);
    try {
      if (isActive) await deactivateOrgAdmin(admin.id);else await reactivateOrgAdmin(admin.id);
      await loadOrgAdmins();
    } catch (err) {
