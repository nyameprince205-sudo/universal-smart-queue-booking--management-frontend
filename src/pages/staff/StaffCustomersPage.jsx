...
function StaffCustomersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return customers.filter(c => c.name?.toLowerCase().includes(term) || c.phone?.replace(/\s/g, "").includes(term.replace(/\s/g, "")));
  }, [customers, search]);
  return <div className="min-h-screen bg-warm-bg p-8">
      ...
      <div className="max-w-3xl mx-auto mt-6">
        <input type="text" placeholder="Search by name or phone number…" value={search} onChange={e => setSearch(e.target.value)} className="w-full rounded-md border border-slate-300 px-4 py-2.5 text-slate-800 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500" />
        ...
        {!loading && filtered.length === 0 && !error && <p className="mt-6 text-slate-400">{search ? "No matching customer found." : "No customers yet."}</p>}

        {!loading && filtered.length > 0 && <div className="mt-4 bg-white rounded-lg border border-slate-200 overflow-hidden">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map(c => <tr key={c.customerId}>
