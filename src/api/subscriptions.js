...
async function listPlans() {
  const {
    data
  } = await apiClient.get("/subscriptions/plans");
  return data;
}
...
async function initializeSubscription(planId) {
  const {
    data
  } = await apiClient.post("/subscriptions/initialize", {
    planId
  });
  return data;
}
async function verifyPayment(reference) {
  const {
    data
  } = await apiClient.get(`/subscriptions/verify/${reference}`);
  return data;
}
async function listPaymentHistory() {
  const {
    data
  } = await apiClient.get("/subscriptions/payments");
  return data;
}
