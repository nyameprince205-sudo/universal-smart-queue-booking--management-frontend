...
function AdminLayout() {
  ...
  return <div className="min-h-screen bg-warm-bg flex">
      ...
      <aside className={`bg-slate-800 bg-gradient-to-b from-slate-800 to-slate-900 text-slate-200 flex flex-col shrink-0 transition-all fixed inset-y-0 left-0 z-40 w-64 transform ${mobileOpen ? "translate-x-0" : "-translate-x-full"} md:relative md:translate-x-0 md:z-auto ${collapsed ? "md:w-16" : "md:w-56"}`}>
        ...
        <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
          ...
          isActive
        }) => `flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition-colors border-l-2 ${isActive ? "bg-slate-700 text-white border-gold-600" : "text-slate-300 hover:bg-slate-700/50 border-transparent"}`}>
